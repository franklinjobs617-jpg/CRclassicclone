...
**测量方法修正:**
- 改用"泰语字符数 ÷ 6.5"估算词等价量（该换算比例为粗略经验估算，非权威标准，明确告知站长存在较大误差范围，仅作方向参考）
- 修正过程中发现自身统计脚本的漏洞：`bestForThExtra`字段实际存储在`lib/data.ts`，渲染时才拼入页面，早期统计只扫描了`page.tsx`文件本身，漏算了这部分——已修正统计方法，如实记录此错误

**内容扩写（两轮，全部基于既有真实数据，非编造新事实）:**
- `app/th/cookie-run-classic-pet-tier-list/page.tsx` — A Tier 4只宠物也做成详情卡片（对齐英文版`A_TIER_DETAILS`结构，之前只有S Tier有）；新增"จับคู่สัตว์เลี้ยงกับสมบัติยังไงให้คุ้ม"搭配说明段（内链Treasure Tier List）；新增"ข้อผิดพลาดที่พบบ่อย"常见错误板块；FAQ从4条扩到5条
- `lib/data.ts` — `PetTier`类型新增`bestForThExtra`可选字段，给7只B Tier宠物补充第二句真实使用场景说明（非填充，例如"เหมาะกับด่านที่มีสิ่งกีดขวางถี่ๆ ด่านโล่งจะได้ผลตอบแทนน้อยกว่า"这类具体使用条件）
- `app/th/page.tsx` — 深化"CookieRun Classic คืออะไร"游戏机制说明（新增League/账号进度双轨说明）；新增"ทำไมต้องเชื่อวิกินี้"信任板块（对应英文首页的trust cards）；新增"เพิ่งเริ่มเล่น ควรทำอะไรก่อน"三步上手指引
- `app/th/cookie-run-classic-codes/page.tsx` — 新增"โค้ดคืออะไร"系统说明段；新增过期码参考表（复用`EXPIRED_CODES`真实数据）；新增"ปัญหาที่พบบ่อยตอนแลกโค้ด"排障板块

**明确决定不继续机械扩写的部分:**
- 首页（约369词等价量/目标1400-1800）、Codes页（约264词等价量/目标900-1200）两轮扩写后仍低于SOP目标区间，判断为**故意不再继续**，理由：这两类页面（2个子页的hub入口页、码表驱动页）信息密度天然低于tier-list类内容页，继续凑字数需要靠重复已说过的内容或空泛陈述，会直接撞上SOP自己反对的"薄内容常见误区"。选择"诚实地薄但每句有信息量"，而非"字数达标但注水"
- Pet Tier List的C Tier（6只宠物）保持简短表格形式不做详情卡——C Tier定位就是"早期用一下、有更好的就换掉"，详略程度对应重要程度是刻意的内容架构选择，非偷懒遗漏（英文版结构同理，只有S/A有独立详情卡）

**验证:**
- `npx tsc --noEmit` 通过
- Pet Tier List修正后实际渲染约904词等价量（含`lib/data.ts`里的`bestForThExtra`），接近1000-1400目标区间

**遗留判断依据（不再用词数估算，改用真实数据）:**
- 首页/Codes页内容量是否"足够"，后续改用2-4周后的GSC真实曝光/点击数据判断，不再依赖不精确的字符换算词数方法

**本轮一个小事故（如实记录）:**
- 写这条CHANGELOG记录时，str_replace操作不慎把"续4"章节的标题行删掉（内容本身没丢，只是丢了标题，导致两节内容粘连），命中的正是SOP第十四章"操作风险与文件版本控制规范"点名警告过的模式。已发现并修复，补记于此作为该条规范在本项目里的又一个实例。

---

## 2026-07-17 (续4) — 多语言SEO专项自检

**触发原因**：站长要求专项自检多语言改动是否充分考虑SEO准则，避免连带影响全站。

**自检方法**：逐项核对hreflang对称性、canonical自指性、sitemap与实际路由一致性、robots.ts覆盖范围、是否误用SOP第十七章禁止的动态locale反模式、Trust页面可达性；并尝试跑完整`next build`（非仅`tsc --noEmit`）。

**发现并修复的真实缺陷:**
- `components/blocks.tsx` — `ThaiFooter`组件此前完全没有链接到`/about`、`/privacy`、`/contact`三个信任页面，泰语用户如果不点"English site"链接，永远不会看到隐私政策入口。已补上三个直接链接（中文标注、指向英文页面，与站内其他未翻译内容的处理方式一致）

**核实通过、无需改动的项目:**
- 三对hreflang声明（首页/Pet Tier List/Codes的en↔th）逐字核对完全对称，无单向声明
- 三个泰语页面canonical均正确自指，无交叉指向风险
- `sitemap.ts`声明的3个`/th`路由与`app/th/`下实际存在的文件一一对应
- `robots.ts`用`allow: '/'`覆盖全站，未误屏蔽`/th`
- 架构验证：项目`next.config.mjs`确认`output: 'export'`（纯静态导出）。这反向证实了此前"不用`[locale]`动态路由+`generateStaticParams`"的选择是正确的——静态导出模式下动态locale方案如果`generateStaticParams`遗漏某语言，会在编译期静默产出空页面而非报错，是比SSR模式更隐蔽的风险，本项目采用的独立文件夹路由完全绕开此类问题
- 确认`next.config.mjs`同时设置了`typescript: { ignoreBuildErrors: true }`——生产构建不因类型错误失败，`tsc --noEmit`检查比实际部署门槛更严格

**本轮未能完成的验证（如实记录，非隐瞒）:**
- 完整`next build`本地未跑通，原因是沙盒环境网络白名单不包含`fonts.googleapis.com`，构建时`next/font`拉取Google Fonts失败——**该报错与本轮代码改动无关**，是环境限制而非代码缺陷，Cloudflare生产构建环境有完整外网访问，预期不受影响，但站长如果想要100%确定性，建议本地或CI环境单独跑一次完整build验证

**验证:**
- 修复ThaiFooter后重新执行 `npx tsc --noEmit`，通过

---

## 2026-07-17 (续3) — 修复：TH页面Header/Footer重复渲染 + 内容深度不足

**背景**：站长实测截图发现两个问题：①`/th/*`页面外层还套着全局英文`SiteHeader`/`SiteFooter`，和页面内自建的`ThaiHeader`/`ThaiFooter`重叠，一个页面出现两套导航；②三个泰语页面内容明显比对应英文页面单薄，有变成Low Value Content的风险。

**问题①根因**：`app/layout.tsx`把`SiteHeader`/`SiteFooter`放在根布局里，包裹所有页面（包括`/th/*`），且Next.js嵌套布局机制无法"取消"父布局已经渲染的内容，必须在组件内部按路径判断。

**Modified — 修复重复导航:**
- `components/site-header.tsx` — 加入路径判断，`pathname`以`/th`开头时`return null`
- `components/site-footer.tsx` — 从服务端组件改为客户端组件（加`'use client'`+`usePathname`），同样逻辑`/th`路径下不渲染

**已知局限（未在本轮解决）:**
- 根`<html lang="en">`是全站唯一声明，Next.js单一根布局架构下无法让`/th`页面单独声明`lang="th"`到`<html>`标签——这需要更大的路由分组重构（把现有全部英文页面移进一个route group）。本轮改为在三个泰语页面各自的`<main>`标签上加`lang="th"`作为折中方案，对可访问性/翻译工具有真实帮助，但不是完整方案，如果后续泰语页面继续扩张到10+页，值得重新评估是否值得做完整的route group重构

**问题②：内容深度不足，重写**

对比英文/泰语版本行数（重写前）：Pet Tier List 英文299行 vs 泰语156行，首页英文521行 vs 泰语93行——确认站长的判断是对的，不是主观感受。

- `app/th/cookie-run-classic-pet-tier-list/page.tsx` — 整页重写：S Tier 5只宠物从"表格里一行小字"扩展成对齐英文版结构的详情卡片（角色定位+2-3句完整说明，参照英文`S_TIER_DETAILS`的`role`+`desc`字段翻译大意，非逐句直译）；新增"สัตว์เลี้ยงสำคัญแค่ไหน"概念说明段；新增"ควรเลือกสัตว์เลี้ยงตัวแรกยังไง"按玩法分场景建议板块；FAQ从2条扩到4条。重写后 **295行，接近英文版299行**，泰语字符从1031增加到2970（约2.9倍）
- `app/th/page.tsx` — 新增"CookieRun Classic คืออะไร"介绍段（复用本轮已核实的游戏史实：2026-06-25全球上线、Kakao版本地化、和Kingdom的区别，内链到`/cookie-run-classic-vs-kingdom`）；新增2条FAQ。155行（原93行），泰语字符645→1293
- `app/th/cookie-run-classic-codes/page.tsx` — 新增3条FAQ（大小写是否敏感、单码限用次数、哪里找新码，最后一条内链官方X账号）。164行（原122行），泰语字符722→1010

**验证:**
- `npx tsc --noEmit` 通过
- 手动核对三个文件行数/泰语字符密度，Pet Tier List已接近英文版体量，首页和Codes页仍略薄于英文版但差距已大幅缩小（首页作为hub页、Codes页作为表格驱动页，本身英文版体量也不是该类页面的典型基准，不强行对齐到521行）

**再次强调（非新增，重申立场）:**
- 本轮扩写内容依然全部基于已验证术语和已有真实数据（PET_TIERS的role/desc翻译大意，非编造新事实），未越过"关键词意图驱动、非逐句翻译"的方法论
- 真人语言校对仍是未完成的阻塞项，扩写内容量不代表准确度提升，这条独立于本轮修复
